...
/**
 * The teacher dashboard's overview figures (M17): KPI counters, the 12-month
 * revenue / enrollment / new-student series, top content, recent sales.
 *
 * Extracted from the controller so the dashboard and its export are the same
 * numbers by construction. A report that quietly disagreed with the screen it
 * was exported from would be worse than no export — the teacher would have no
 * way to tell which one to trust.
 *
 * Year scoping: content models scope themselves through BelongsToAcademicYear;
 * students, orders and revenue key on the student's pinned year, so a paid order
 * counts toward the year its buyer belongs to.
 */
class TeacherOverviewQuery
{
    /**
     * @param  int|null  $yearId  active academic year, or null for tenant-wide
     * @return array<string, mixed>
     */
    public function build(int $tenantId, ?int $yearId): array
    {
