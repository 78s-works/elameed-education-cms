...
/*
 * ─────────────────────────────────────────────────────────────────────────────
 * THE SEED, IN ONE FILE
 *
 * Everything the database is populated with lives here, in the order it runs:
 *
 *   GuardsDemoSeeding        the production guard demo seeders check first
 *   PermissionCatalogSeeder  code-owned permission catalog -> database (M20)
 *   RoleTemplateSeeder       the role blueprints a new tenant is stamped from
 *   RbacTestSeeder           just those two — what the test suite boots
 *   NotificationCatalogSeeder  notification types, templates, AR/EN copy
 *   DatabaseSeeder           the entry point: catalogs, then the demo academies
 *   AhmedTammamAcademySeeder the large demo academy (guarded off production)
 *
 * They stay separate CLASSES because the suite and the runbooks seed them
 * individually — `$this->seed(NotificationCatalogSeeder::class)` in five test
 * files, and `db:seed --class=NotificationCatalogSeeder` in the notifications
 * runbook. Folding them into one class would make every one of those runs seed
 * two full demo academies. `composer.json` maps this directory with a classmap
 * so each class is still found by name from this one file.
 * ─────────────────────────────────────────────────────────────────────────────
 */
