...
- **Auth transport (🔧 2026-08-19 — httpOnly cookie):** browsers no longer store the token in JS. `login` / `otp/verify` / `parent/magic` set the token in an **httpOnly, Secure, SameSite=Lax cookie** (`elameed_token`) — and still echo `token` in the body for non-browser Bearer clients. `AuthenticateWithTokenCookie` (api group, ordered before `auth:sanctum` via the middleware priority list) reads the cookie into the `Authorization` header, so cookie and Bearer are interchangeable. `logout` clears the cookie. Cookie-authenticated **mutations** (POST/PUT/PATCH/DELETE) must carry a double-submit CSRF token — the readable `XSRF-TOKEN` cookie echoed in the `X-CSRF-Token` header (`VerifyCookieCsrf`, → `419` on mismatch); Bearer-header clients are exempt. Requires the API to be same-origin with the SPA (server proxy of `/api`, see frontend `docs/api-proxy-setup.md`). CORS `supports_credentials=true`.
